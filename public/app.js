...
function createAttendee(){attendeeSeq++;const card=document.createElement('article');card.className='delegate-card';card.dataset.id=attendeeSeq;card.innerHTML=`<div class="delegate-head"><h3>Attendee</h3><button type="button" class="remove-btn">Remove</button></div><div class="person-fields"></div><div class="form-grid" style="margin-top:16px"><label>Voting Status *<select class="voting-status" required><option value="">Select voting status</option><option value="Primary Member">Primary Member</option><option value="Observer">Observer</option></select></label><label>Check-in Date *<input class="conference-check-in" type="date" required></label><label>Check-out Date *<input class="conference-check-out" type="date" required></label></div>${flightFields('inbound')}${flightFields('outbound')}`;commonFields(card.querySelector('.person-fields'));document.querySelector('#attendeeList').appendChild(card);card.querySelector('.remove-btn').addEventListener('click',()=>{card.remove();renumber();updateSummary()});renumber();bindPhoto(card);bindFlightToggles(card);updateSummary()}

let activePhotoInput=null,cameraStream=null,capturedBlob=null;
const cameraModal=document.querySelector('#cameraModal'),cameraVideo=document.querySelector('#cameraVideo'),cameraCanvas=document.querySelector('#cameraCanvas'),cameraError=document.querySelector('#cameraError');
function stopCamera(){if(cameraStream){cameraStream.getTracks().forEach(track=>track.stop());cameraStream=null}if(cameraVideo)cameraVideo.srcObject=null}
function closeCamera(){stopCamera();capturedBlob=null;cameraModal.classList.add('hidden');cameraCanvas.classList.add('hidden');cameraVideo.classList.remove('hidden');document.querySelector('#capturePhoto').classList.remove('hidden');document.querySelector('#retakeCapture').classList.add('hidden');document.querySelector('#usePhoto').classList.add('hidden')}
async function optimiseImage(file){return new Promise((resolve,reject)=>{const img=new Image(),url=URL.createObjectURL(file);img.onload=()=>{try{const max=1200,scale=Math.min(1,max/Math.max(img.width,img.height)),canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(img.width*scale));canvas.height=Math.max(1,Math.round(img.height*scale));canvas.getContext('2d').drawImage(img,0,0,canvas.width,canvas.height);canvas.toBlob(blob=>{URL.revokeObjectURL(url);if(!blob)return reject(new Error('The selected image could not be processed.'));resolve(new File([blob],`headshot-${Date.now()}.jpg`,{type:'image/jpeg',lastModified:Date.now()}))},'image/jpeg',.82)}catch(e){URL.revokeObjectURL(url);reject(e)}};img.onerror=()=>{URL.revokeObjectURL(url);reject(new Error('Please choose a valid image file.'))};img.src=url})}
async function setPhoto(input,file){const processed=await optimiseImage(file),dt=new DataTransfer();dt.items.add(processed);input.files=dt.files;const reader=new FileReader();reader.onload=()=>{input.dataset.preview=reader.result;const field=input.closest('.photo-field'),preview=field.querySelector('.photo-preview');preview.innerHTML=`<img src="${reader.result}" alt="Headshot preview">`;field.querySelector('.photo-preview-actions').classList.remove('hidden');updateSummary()};reader.readAsDataURL(processed)}
async function openCamera(input){activePhotoInput=input;cameraError.classList.add('hidden');cameraModal.classList.remove('hidden');if(!navigator.mediaDevices?.getUserMedia){cameraError.textContent='Camera access is not supported in this browser. Please upload a photo instead.';cameraError.classList.remove('hidden');return}try{cameraStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'user'},width:{ideal:1280},height:{ideal:960}},audio:false});cameraVideo.srcObject=cameraStream;await cameraVideo.play()}catch(e){cameraError.textContent='Camera access was denied or is unavailable. Check browser permissions, or upload a photo instead.';cameraError.classList.remove('hidden')}}
function bindPhoto(scope){scope.querySelectorAll('.person-photo').forEach(input=>{const field=input.closest('.photo-field');input.addEventListener('change',async()=>{if(!input.files[0])return;try{await setPhoto(input,input.files[0])}catch(e){alert(e.message);input.value=''}});field.querySelector('.upload-photo').addEventListener('click',()=>input.click());field.querySelector('.take-photo').addEventListener('click',()=>openCamera(input));field.querySelector('.retake-photo').addEventListener('click',()=>openCamera(input));field.querySelector('.remove-photo').addEventListener('click',()=>{input.value='';delete input.dataset.preview;field.querySelector('.photo-preview').textContent='Photo preview';field.querySelector('.photo-preview-actions').classList.add('hidden');updateSummary()})})}
document.querySelector('.camera-close').addEventListener('click',closeCamera);cameraModal.addEventListener('click',e=>{if(e.target===cameraModal)closeCamera()});document.querySelector('#cameraUploadFallback').addEventListener('click',()=>{const input=activePhotoInput;closeCamera();input?.click()});document.querySelector('#capturePhoto').addEventListener('click',()=>{if(!cameraStream)return;cameraCanvas.width=cameraVideo.videoWidth||1280;cameraCanvas.height=cameraVideo.videoHeight||960;cameraCanvas.getContext('2d').drawImage(cameraVideo,0,0,cameraCanvas.width,cameraCanvas.height);cameraCanvas.toBlob(blob=>capturedBlob=blob,'image/jpeg',.9);cameraVideo.classList.add('hidden');cameraCanvas.classList.remove('hidden');document.querySelector('#capturePhoto').classList.add('hidden');document.querySelector('#retakeCapture').classList.remove('hidden');document.querySelector('#usePhoto').classList.remove('hidden');stopCamera()});document.querySelector('#retakeCapture').addEventListener('click',()=>openCamera(activePhotoInput));document.querySelector('#usePhoto').addEventListener('click',async()=>{if(!capturedBlob||!activePhotoInput)return;await setPhoto(activePhotoInput,new File([capturedBlob],`camera-${Date.now()}.jpg`,{type:'image/jpeg'}));closeCamera()});
...
form.addEventListener('submit',async e=>{e.preventDefault();const error=form.querySelector('#error'),success=form.querySelector('#success');error.classList.add('hidden');success.classList.add('hidden');if(!form.checkValidity()){form.reportValidity();return}const button=e.submitter;button.disabled=true;button.dataset.originalLabel=button.textContent;button.textContent=window.FEDUSA_I18N?.translate('Submitting…')||'Submitting…';try{let payload;if(currentPath==='affiliate'){const attendees=[];for(const card of document.querySelectorAll('.delegate-card')){const checkIn=card.querySelector('.conference-check-in').value,checkOut=card.querySelector('.conference-check-out').value;if(checkOut<=checkIn)throw new Error('Conference check-out must be after the check-in date.');attendees.push({...await personPayload(card),votingStatus:card.querySelector('.voting-status').value,conferenceDates:{checkIn,checkOut},inboundFlight:flightPayload(card,'inbound'),outboundFlight:flightPayload(card,'outbound')})}payload={registrationType:'affiliate',affiliate:document.querySelector('#affiliateName').value,primaryContact:{fullName:document.querySelector('#repName').value.trim(),role:document.querySelector('#repRole').value.trim(),email:document.querySelector('#repEmail').value.trim(),mobile:document.querySelector('#repPhone').value.trim()},attendees}}else{const checkIn=document.querySelector('#conferenceCheckIn').value,checkOut=document.querySelector('#conferenceCheckOut').value;if(checkOut<=checkIn)throw new Error('Conference check-out must be after the check-in date.');payload={registrationType:'main',delegate:{...await personPayload(document.querySelector('#mainPerson')),votingMember:document.querySelector('#mainVoting').value,accommodation:{required:document.querySelector('#accommodationRequired').value},conferenceDates:{checkIn,checkOut},inboundFlight:flightPayload(form,'inbound'),outboundFlight:flightPayload(form,'outbound')}}}payload.language=window.FEDUSA_I18N?.language||'en';const response=await fetch('/api/register',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const data=await response.json();if(!response.ok)throw new Error(data.error||'Registration failed.');success.textContent=`Registration successful. ${data.delegates.length} attendee record(s) created.`;success.classList.remove('hidden');showPromo(currentPath)}catch(err){error.textContent=err.message;error.classList.remove('hidden')}finally{button.disabled=false;button.textContent=button.dataset.originalLabel||window.FEDUSA_I18N?.translate('Submit Registration')||'Submit Registration'}})
