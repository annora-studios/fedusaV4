...
export default async(req)=>{if(req.method!=='POST')return json({error:'Method not allowed'},405);try{
 ...
 for(const person of people){
  ...
  const delegateId=id('DEL'),badgeId=id('BADGE');
  const votingStatus=body.registrationType==='affiliate'?(person.votingStatus==='Primary Member'?'Primary Member':'Observer'):(person.votingMember==='Yes'?'Primary Member':'Observer');
  const voting=votingStatus==='Primary Member'?'Yes':'No';
