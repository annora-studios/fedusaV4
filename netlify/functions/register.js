...
export default async(req)=>{if(req.method!=='POST')return json({error:'Method not allowed'},405);try{
 ...
 for(const person of people){
  ...
  const votingQrValue=voting==='Yes'?JSON.stringify({type:'FEDUSA_VOTING',delegateId,name:person.fullName,voting:true}):null;
  ...
  const ext=upload.mime==='image/png'?'png':upload.mime==='image/webp'?'webp':'jpg',headshotKey=`headshots/${delegateId}.${ext}`,badgeQrKey=`qrcodes/${delegateId}-badge.png`,votingQrKey=votingQrValue?`qrcodes/${delegateId}-voting.png`:null;
  await assets.set(headshotKey,upload.buffer,{metadata:{contentType:upload.mime}}); await assets.set(badgeQrKey,await makeQr(badgeQrValue),{metadata:{contentType:'image/png'}}); if(votingQrKey)await assets.set(votingQrKey,await makeQr(votingQrValue),{metadata:{contentType:'image/png'}});
  const row={registrationId,registrationType:body.registrationType,delegateId,badgeId,createdAt:now(),updatedAt:now(),affiliate:body.registrationType==='affiliate'?body.affiliate:'',primaryContact:body.registrationType==='affiliate'?body.primaryContact:null,fullName:person.fullName,jobTitle:person.jobTitle||'',email:person.email,mobile:person.mobile,normalizedEmail:normalizeEmail(person.email),normalizedMobile:normalizeMobile(person.mobile),dietary:person.dietary||'None',accessibility:person.accessibility||'',shirtJacketSize:person.shirtJacketSize,votingMember:voting,accommodation:body.registrationType==='main'?(person.accommodation||{required:'No'}):{required:'No'},conferenceDates:person.conferenceDates,inboundFlight:person.inboundFlight||{status:'Not booked'},outboundFlight:person.outboundFlight||{status:'Not booked'},headshotKey,badgeQrKey,votingQrKey,badgeQrValue,votingQrValue,checkedInAt:null,checkedInBy:null,checkinMethod:null,badgePrintedAt:null};
  const key=`delegate/${delegateId}`;await records.setJSON(key,row);delegateIndex.push(key);delegateIds.push(delegateId);delegateNames.push(person.fullName);created.push({delegateId,badgeId,fullName:person.fullName,votingQrGenerated:Boolean(votingQrKey)});
 }
